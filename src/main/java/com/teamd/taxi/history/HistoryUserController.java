...
public class HistoryUserController {
    ...
    List<TaxiOrder> getListTaxiOrder(int pageNumber,String filter){
        ...
        for(int i=1;i<=7;i++){
            ...
            route.setStartTime(new Date(Calendar.getInstance(Locale.ENGLISH).getTimeInMillis() - 1001100));
            route.setCompletionTime(new Date(Calendar.getInstance(Locale.ENGLISH).getTimeInMillis()));
            ...
            route.setStartTime(new Date(Calendar.getInstance(Locale.ENGLISH).getTimeInMillis()));
