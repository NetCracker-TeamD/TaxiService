...
public class AdditionalOptionsForUserExcelView extends AbstractExcelView{
    @Override
    protected void buildExcelDocument(Map model, HSSFWorkbook workbook,
                                      HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        response.setContentType("application/vnd.ms-excel");
        response.setHeader("Content-disposition", "attachment; filename=MostPopularAdditionalOptionForUser.xls");
        HSSFSheet excelSheet = workbook.createSheet("Most popular additional car options for each customer user");
    ...
    public void setExcelHeader(HSSFSheet excelSheet,HSSFWorkbook workbook) {
        ...
        excelHeader.createCell(1).setCellValue("Feature name");
