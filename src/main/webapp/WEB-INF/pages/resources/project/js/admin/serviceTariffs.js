...
function startEditTariff(node) {
    ...
    var priceByTime = record.find(':nth-child(3)').eq(0);
    var priceByDistance = record.find(':nth-child(4)').eq(0);
    var minPrice = record.find(':nth-child(5)').eq(0);
    ...
    var priceByTimeVal = priceByTime.eq(0).text();
    priceByTime.eq(0).text('');
    priceByTime.eq(0).html(textInput);
    priceByTime.eq(0).find(':nth-child(1)').val(priceByTimeVal);

    var priceByDistanceVal = priceByDistance.eq(0).text();
    priceByDistance.eq(0).text('');
    priceByDistance.eq(0).html(textInput);
    priceByDistance.eq(0).find(':nth-child(1)').val(priceByDistanceVal);

    var minPriceVal = minPrice.eq(0).text();
    minPrice.eq(0).text('');
    minPrice.eq(0).html(textInput);
    minPrice.eq(0).find(':nth-child(1)').val(minPriceVal);
